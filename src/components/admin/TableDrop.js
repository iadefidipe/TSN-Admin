...
function TableDrop({ setOpenModal, data, client, positionID, setRequests }) {
  ...
  const getData = async () => {
    const status = await userService.getPostionmatchStatus()
  ...
  const handleSubmit = async () => {
    ...
    if (!matchID) return
    const data = {
      position_match_status_id: matchID,
    }
    try {
      setLoading(true)
      await userService.updateMatch(client, positionID, data)
      // if (request) {
              ...
              >
                {status.map((item, index) => (
                  ...
                  key={index}
                    ...
                    onClick={() => {
                      setMatch(item.position_match_status)
                      setMatchID(item.position_match_status_id)
                    }}
                  >
                    {item.position_match_status}
                  </div>
                ))}
              </div>
            )}
